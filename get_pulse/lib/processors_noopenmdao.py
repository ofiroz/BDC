...
class findFaceGetPulse(object):
    ...
    # TODO: changed from (self,cam)
    def run(self, cam): 
        self.times.append(time.time() - self.t0)
        self.frame_out = self.frame_in
        self.gray = cv2.equalizeHist(cv2.cvtColor(self.frame_in,
                                                  cv2.COLOR_BGR2GRAY))
        # cv2.imshow("Original",self.gray)
        # cv2.waitKey(50)                                         
        col = (100, 255, 100)
        if self.find_faces:
            cv2.putText(
        ...
        if L > 10:
            ...
            try:
                ...
                print("Chatched: ValueError: attempt to get argmax of an empty sequence! --> processors_noopenmdao, line 210")
                return
