...
def save_video(video_tensor):
    ...
    # TODO: changed in order to show right away
    '''
    fourcc = cv2.VideoWriter_fourcc('M','J','P','G')
    [height,width]=video_tensor[0].shape[0:2]
    writer = cv2.VideoWriter("sample4_mon_mag.mp4", fourcc, 30, (width, height), 1)
    for i in range(0,video_tensor.shape[0]):
...
# magnify motion
def magnify_motion(video_name,low,high,levels=3,amplification=5):
...
if __name__=="__main__":
    magnify_motion("sample4_resized.mp4",0.4,1)#was 0.4,1
    print("DONE in motion mag")
